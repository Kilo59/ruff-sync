...
```text
[1. Intake Context] ➔ [2. Sync & Branch] ➔ [3. Test First (TDD)] ➔ [4. Implement] ➔ [5. Validate] ➔ [6. Sync Docs/Skills] ➔ [7. Commit & PR]
```
...
## 5. Validate the Quality Pipeline

Run the 4-step quality suite in order. All commands **must** succeed:
...
## 6. Synchronize Documentation, Skills, & Assets
...
## 7. Commit & Open Pull Request

1. **Commit with Conventional Commits**:
   ```bash
   git add <files>
   git commit -m "feat(cli): add --validate-toml check before write (#123)"
   ```
...
3. **Open Pull Request via `gh` CLI**:
   ```bash
   gh pr create \
