...
def lock_file_poetry_version() -> Version:
    ...
    captured_version: re.Match[str] | None = re.search(
        r"#.*generated by Poetry (?P<version>\d\.\d\.\d)",
        poetry_lock.read_text().splitlines()[0],
    )
...
def latest_poetry_version(lock_file_poetry_version: Version) -> Version:
    ...
    if lock_file_poetry_version < latest_version:
        # warning instead of error because we don't want to break the build
        # whenever a new version of poetry is released
        warnings.warn(
            f"The latest version of poetry is {latest_version} but the poetry.lock file"
            f" was generated using {lock_file_poetry_version}."
...
def test_lockfile_poetry_version(
    lock_file_poetry_version: Version, latest_poetry_version: Version
):
    """
    Ensures that the poetry.lock file was generated using a recent version of poetry.
    Important because the lockfile format or dependency resolving strategy could change
    between versions.
    """
